...
df_wieghted <- as.data.frame(do.call(rbind, lapply(loo_list2, as.data.frame)))
df_simple <- as.data.frame(do.call(rbind, lapply(loo_list, as.data.frame)))


df_simple$Model <- "simple"
df_wieghted$Model <- "weighted"
df_combined <- rbind(df_simple, df_wieghted)

df_combined %>% 
  ggplot(aes(x= `X[[i]]`, color = Model, fill = Model, alpha = 0.5)) +
  geom_density() +
  xlab("elpd_loo") 
...
```{r}
ggplot() +
  geom_histogram(aes(df_simple, fill = X[[i]]), data = df_simple) +
  geom_histogram(aes(df_wieghted, fill = X[[i]]), data = df_wieghted)
```
```{r}
df_wieghted
```
...
#draws_df_emp %>% 
  ...
  ggplot(draws_df_emp, aes(x = W1, y = fct_reorder(as.factor(ID), W1))) +
    geom_density_ridges() +
   #geom_vline(xintercept = 0.3, color = "red", linetype = "dashed", size = 1) +
    labs(title = "Distribution of Bias by ID",
         x = "Bias",
    ...
    ggplot(draws_df_emp, aes(x = W2, y = fct_reorder(as.factor(ID), W2))) +
    geom_density_ridges() +
   #geom_vline(xintercept = 0.3, color = "red", linetype = "dashed", size = 1) +
    labs(title = "Distribution of Bias by ID",
         x = "Bias",
...
for (i in IDS){
  ...
  loo_list2 <- c(loo_list2, mean(b[,1]))
